...
PixelPoint PriorityNeighbours::Remove() {
  // complete your implementation below
  PixelPoint ret=points[0];
  double rdist=refcolor.dist(points[0].color);
  for (int i=points.size()-1;i>0;--i)
  {
    PixelPoint nxt=points[i];
    if (refcolor.dist(nxt.color) < rdist)
    {
      ret=nxt;
    }
    else if (refcolor.dist(nxt.color) == rdist)
    {
      if (nxt.y<ret.y)
      {
        ret=nxt;
